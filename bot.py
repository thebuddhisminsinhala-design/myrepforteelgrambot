...
For ALL text nodes, use EXACTLY TWO `<tspan>` elements:
...
async def render_svg_with_playwright(svg_code: str) -> bytes:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Strict 15-second timeout on content loading to prevent hangs
            await page.set_content(svg_code, wait_until="domcontentloaded", timeout=15000)
            await asyncio.sleep(0.5)  # Brief delay for internal font rendering
...
# ================= REQUEST PROCESSOR WITH HARD TIMEOUT =================
async def process_diagram_request(update: Update, context: ContextTypes.DEFAULT_TYPE, query_text: str):
    ...
    try:
        # Wrap execution in 45-second strict timeout
        async with asyncio.timeout(45):
            png_bytes, caption = None, ""
            for attempt in range(2):
                try:
                    # Run Gemini generation in a separate thread to keep event loop free
        ...
        await update.message.reply_text("⏱️ Diagram generation took too long. Please try again with a simpler request!")
    except Exception as e:
...
def main():
    ...
    logger.info(f"⚡ {BOT_NAME} Bot started with strict timeout safeguards!")
